...
@section('content')
    <div class="card">
        <div class="card-header border-bottom d-flex align-items-center justify-content-between">
            <h5 class="card-title mb-0">@lang('nav.articles')</h5>
            {{-- check if auth user has ability to create  --}}
            @if (auth('admin')->user()->hasAbilityTo('create articles'))
                <a href="{{ route('admin.articles.create') }}" class="btn btn-primary"><i
                        class="bx bx-plus me-0 me-lg-2"></i><span
                        class="d-none d-lg-inline-block">{{ __('add new articles') }}</span></a>
            @endif
        </div>
    ...
    <script>
        $('document').ready(function() {
            ...
            let datatable = $('.datatables-categories').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{!! route('admin.articles.articles_list') !!}",
                columns: [{
                        data: 'image',
                        name: 'image'
                    },
